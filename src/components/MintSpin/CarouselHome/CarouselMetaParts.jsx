...
import { resultImagesMetaParts } from "./CarouselData";
...
const CarouselMetaParts = () => {
  ...
  var reduced = resultImagesMetaParts.reduce(function (filtered, option) {
    if (option.deg === items?.metaPartVal) {
      var someNewValue = {
        name: option.name,
  ...
  return (
    <div className="carousel-background" style={style}>
      {isDesktop && !items.checkMetaParts ? (
        <>
          <div
            style={{
              display: "flex",
...
export default CarouselMetaParts;
